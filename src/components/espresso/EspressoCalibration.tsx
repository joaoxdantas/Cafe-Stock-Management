...
export default function EspressoCalibration() {
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-end mb-[24px]">
        <div>
          <h2 className="text-[24px] font-normal tracking-[-0.5px]">{t('espressoCalibration')}</h2>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex justify-center p-2 sm:p-4 z-50 backdrop-blur-sm overflow-y-auto">
          <div className="bg-cafe-surface border border-cafe-border w-full max-w-4xl h-fit my-auto flex flex-col rounded-[8px] text-left shadow-2xl relative">
            <button title="Fechar" onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-cafe-text-dim hover:text-cafe-text z-10 transition-colors">
              <Plus className="w-5 h-5 rotate-45" />
            </button>
            <div className="shrink-0 p-[20px] border-b border-cafe-border bg-cafe-bg/50">
              <h2 className="text-[18px] m-0 tracking-[-0.5px] font-medium">{t('newMachineTest')}</h2>
            </div>
            ...
            <div className="p-[16px_20px] border-t border-cafe-border flex justify-end gap-[12px] bg-cafe-bg/30 rounded-b-[8px]">
              ...
              <button form="test-form" type="submit" className="p-[8px_16px] bg-cafe-accent text-black border-none rounded-[4px] text-[12px] font-semibold cursor-pointer hover:opacity-90">{t('saveTest')}</button>
            </div>
          </div>
        </div>
      )}
